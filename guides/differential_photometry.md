...
After Generate finishes, Source Results lists the known variables (and discovery candidates) found in the field. Selecting a row shows that target on the image with its automatically selected comparison stars (sticky rows at the bottom of Source Results) and updates the light curve to that selection. Use **Scan Comps** to build a mag/color/distance candidate pool, favourite comparison stars, browse unordered combination light curves, and apply a chosen set (comps appear in sticky Source Results rows). Filter → **Overview** plots the selected target in every filter plus its comparison stars and check star (legend on, no period fit) for a Phoranso/AAVSO-style QC view; right-click any series to change Icon and Color. Export → Animation can reveal all Overview series together over time. Export → Target Field Animation crops the selected target from every frame (star field size, Crop + Align or Align + Crop keeps the target centered and can apply a left–right, up–down, or 180° crop flip when matched stars agree, stretch, and GIF or MP4 timing), measures an ensemble of non-central stars directly in the aligned crop to remove common cloud and transparency changes, uses the catalog comparison measurements or background matching when the crop is sparse, and builds a 16:9 GIF or MP4 with a star-field crop above a full-width still of the on-screen light curve (icon, color, and a moving vertical scanner). The export window is a visual editor: a large Combined / Star Field / Light Curve preview on the left, composition, marker, and export settings in a right-hand inspector, Advanced fit and separate-file options collapsed by default, and a footer with the output summary plus Export Animation. Use **Aperture Editor** (above) when you want a fully manual, independent reduction instead of the automatic ensemble.
